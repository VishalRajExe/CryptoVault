        ...
        {loading ? (
          ...
          >
            {coins.map((c, idx) => {
              ...
              const spark = generateCandles(24, c.currentPrice || 100, 0.02).map((x) => x.close);
              return (
                <motion.div
                  key={c.id}
                ...
                >
                  ...
                  <div className="h-10 -mx-1 mb-2 relative z-10">
                    <Sparkline data={spark} width={240} height={40} color={palette[idx % palette.length]} />
                  </div>
                  ...
                  <button
                    onClick={() => setTradeCoin(c)}
                    className="w-full mt-4 py-2 rounded-lg bg-white/[0.05] border border-white/10 text-ink text-xs font-display font-semibold hover:bg-white/[0.08] transition-colors"
                  >
                    Trade
                  </button>
                </motion.div>
